...
def random_position(body):
    options = []
    for x in range(0, (frame_size_x // square_size) * square_size - square_size + 1, square_size):
        for y in range(0, (frame_size_y // square_size) * square_size - square_size + 1, square_size):
            if [x,y] not in body:
                options.append([x,y])
    if len(options) == 0:
        return [-1, -1]
...
def init_vars():
    ...
    head_pos = [120, 60]
    snake_body = [[120, 60]]
    food_pos = random_position(snake_body)
...
while True:
    ...
    length = (frame_size_x // square_size) * square_size
    width = (frame_size_y // square_size) * square_size
    ...
    while x < length:
        while y < width:
            pygame.draw.rect(game_window, gray, pygame.Rect(x, y, square_size, square_size))
            y += 2*square_size
        x += square_size
        if offset == 0:
            y = square_size
